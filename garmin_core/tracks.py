"""
garmin_core.tracks — the RSD track-inventory builder as a callable library.

This is legacy/rsd_tracks_to_geojson.py with faithful changes only:
  1. The config block now arrives via TracksConfig.to_globals(), injected by
     build_track_inventory(). Default TracksConfig == original literals.
  2. main() becomes build_track_inventory(input_folder, output_path, config,
     progress_cb); INPUT_FOLDER/OUTPUT_PATH are arguments, not globals.
  3. SystemExit (a script idiom) becomes ValueError so callers/servers can
     handle it; the messages are unchanged.

All functions below are sliced verbatim from the original.
"""
...
def build_track_inventory(input_folder, output_path=None, config=None,
                          *, progress_cb=None):
    """Build/extend a GeoJSON track inventory for a folder of RSD files.

    Faithful to legacy main(): same discovery, resume, skip and write
    semantics. Returns a summary dict.

    Args:
        input_folder: folder to scan for .RSD files.
        output_path:  geojson path; default <input_folder>/rsd_tracks.geojson.
        config:       TracksConfig; defaults reproduce the original script.
        progress_cb:  optional callable(done:int, total:int, name:str).
    """
    cfg = config if config is not None else TracksConfig()
    ...
    if not rsd_files:
        raise ValueError(f"No .RSD files found under {input_folder}")
    ...
    for done, rsd_file in enumerate(rsd_files, 1):
        if progress_cb is not None:
            try:
                progress_cb(done, total, rsd_file.name)
            except Exception:
                pass
    ...
    return {
        ...
        "skipped": [(str(p), r) for p, r in skipped],
    }
